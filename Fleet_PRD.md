# PRD v1 (Consolidated) — AI Fleet Disruption & Voyage Recovery Orchestrator
...
**Implementation:** `reference_app/` (via Qwen Code / Qwen coder model → Google AI Studio Build mode)  
...
* Reduce P90 recovery-plan time from 286 minutes to <120 minutes.
* Reduce manual duplicate reconciliation effort from 14.8% to <2%.
* Reduce late-constraint plan revision rate from 22.1% to <5%.
* Make every surfaced AI-assisted fact traceable to source evidence with explicit freshness, confidence, and unavailability markers.
...
* **No autonomous navigation command or route planning authority.** The AI cannot write or execute navigational commands.
* **No autonomous recovery action execution** without Master/Fleet Operations Centre approval.
* **No commercial chartering negotiation, crew HR decisions, or physical maintenance work authorization** (CMMS informs planning only).
* **No reliance on cloud GenAI for safe navigation.** Essential vessel functions must operate deterministically offline.
* **No autonomous duplicate merging.** Duplicate events require explicit human disposition or deterministic idempotency keys.
* **No use of real sensitive/regulated production data in this demo.** Synthetic/mock data only.
* **No silent inference of unknown data permissions.** CONDITIONAL or RESTRICTED data is not used by default; availability does not equal permission.
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **Master (Vessel)** | Yes | Yes | Yes (Nav) | Yes (within COLREGs) | Yes | Yes | Yes |
| **Fleet Operations Desk** | Yes | Yes | Yes (Commercial) | No | Yes | Yes | Yes |
| **Bridge Team** | Yes | Yes | No | No | Yes | Yes | No |
| **Port/Cargo/Crew Coord.** | Yes | Yes (constraints only) | No | No | Yes | No | No |
| **Safety/Compliance Obs.** | Yes | No | No | No | Yes | No | Yes |
| **Audit Observer** | Yes | No | No | No | Yes | No | Yes |

*Note: The UI is not the security boundary in a production system; server/API authorization with vessel/shore tenant isolation would be required. The reference app centralizes the role/action policy so forbidden writes (especially navigation commands) are testable.*
...
| **FR-005** | Show duplicate events with match evidence; persist explicit disposition; never auto-execute duplicate actions. | No automatic confirmed disposition/action path exists. | Stages 4, 5, 7 |
...
| **FR-010** | Master or Fleet Operations Desk only can approve a recovery plan; Safety/Compliance Observer & Audit Observer are read-only except export. | Negative authorization tests pass. | Stages 5, 7 |
...
* **NFR-003 Authorization:** forbidden role/action combinations (especially AI writing navigation commands) are blocked by centralized policy; production would require equivalent server-side enforcement with vessel/shore tenant isolation.
...
* **NFR-006 Offline Survival:** essential vessel-side workflow continues seamlessly during shore/cloud satellite outages.
* **NFR-007 Idempotency:** at-least-once event delivery must be reconciled deterministically without duplicate operational actions.
* **NFR-008 Cost Constraint:** satellite and AI usage must not exceed baseline + 10%; verbose LLM payloads must be bounded and throttled.
* **NFR-009 Accessibility:** semantic HTML, labels and keyboard-usable controls; production requires formal accessibility testing.
* **NFR-010 Recoverability:** state export provides workshop evidence; production persistence/backup/RPO/RTO are open decisions.
...
**Permission invariant:** availability is not permission. Navigation commands are **PROHIBITED** for AI write authority. The demo uses synthetic fixtures only. Production access/reuse/training permissions require explicit Stage 6 evidence and approval. UNKNOWN, CONDITIONAL or RESTRICTED never means "allowed by default."
...
| Minimum-criteria context extraction F1 | ≥95% on versioned adjudicated benchmark |
...
| Duplicate event → duplicate operational action | 0 (Strict idempotency) |
...
* **Unauthorized execution / Nav command** → centralized role/action policy + negative tests + PROHIBITED write boundary; Master/Fleet Operations Desk approval required.
...
1. **Disruption Queue / Fleet Command Center** (Queue, connectivity state, dedup status)
2. **Vessel Workspace + Evidence Drawer** (telemetry, AIS, weather, port, cargo, crew, CMMS, freshness markers)
3. **Impact Assessment & Constraint Validation** (Deterministic solver output)
4. **Recovery Option Review & Approval Console** (Master approval gate, rationale capture)
5. **Execution Monitor & Replan View** (Action tracking, post-outage sync)
6. **Evaluation Dashboard** (Metrics, thresholds, golden-set regression)
7. **Risk & Controls** (Risk mapping, owner/status)
8. **Audit Explorer / Export** (State export, version history, provenance chain)
9. **Offline Mode Indicator & Sync Status** (Edge state visibility, reconnect reconciliation)
...
* [ ] Unauthorized-action negative tests pass (especially AI nav command attempts).
* [ ] Minimum-criteria incomplete/complete disruption cases pass.
...
* [ ] Calculated threshold tests pass (median <45 min, P90 <120 min, duplicate rate <2%, etc.).
...
* Weather/ocean provider SLA and license.
* Port API permissions and semantic mapping approval.
* Crew data purpose limitation, lawful basis, minimization, retention, residency and role access (Privacy/Security).
* Cargo/customer data minimization and tenant isolation.
...
* Every product feature must map to a domain capability/rule from Stage 5 and permitted data from Stage 6. 
* Every AI behavior must map to an evaluation scenario/threshold and a risk/oversight control from Stage 7. 
* Every architecture/component choice must map to the selected option and preliminary ADRs from Stage 8.
* The PRD may not introduce new high-impact capabilities that were not qualified in Stage 4.

---
**End of Consolidated PRD v1**